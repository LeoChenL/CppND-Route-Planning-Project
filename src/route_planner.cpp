...
/*
Generate the final path by retracing the path nodes
...
// Calculate H value (distance to the end node)
float RoutePlanner::CalculateHValue(RouteModel::Node *node) {
  return node->distance(*end_node);
}

// find the minimum cost(h+g value) node in the open list
...
/*
Find and add current node neighbors into the openlist
Set neighbors's parent node as current node
Calculate those neighbors' h and g value
